...
| Tunnel credentials JSON | `cloudflared` (historical - see `TUNNEL.md`) | file on origin host, referenced by `config.yml` | same, deployed to origin host(s) out-of-band | Only relevant while the Tunnel/VPS rollback path is still kept warm; see `TUNNEL.md` |
