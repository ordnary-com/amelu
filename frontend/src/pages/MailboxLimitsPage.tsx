...
export function MailboxLimitsPage() {
  ...
  useEffect(() => {
    if (!mailboxId) return;
    api.getMailboxLimits(mailboxId).then((l) => {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setBusy(true);
    try {
      const r = await api.updateMailboxLimits(mailboxId, {
      ...
      setError(err instanceof ApiError ? err.message : "Could not save limits");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="settings-form">
      <h1>Mailbox Limits</h1>
      <p>Set absolute resource caps on this mailbox.</p>
      <p className="light">
